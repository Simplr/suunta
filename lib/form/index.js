...
export function formDataToJson(form) {
    const formDataJson = new FormData(form)
        ...
        .reduce((/** @type { Record<string, unknown> } */ acc, [key, value]) => {
            ...
            const emptyArray = key.match(/\[\]/);
            if (emptyArray) {
                const singularKey = key.replace('[]', '');
                if (!Array.isArray(acc[singularKey])) {
                    acc[singularKey] = [];
                }
                /** @type { Array<unknown> } */ (acc[singularKey]).push(transformedValue);
                return acc;
            }
    ...
    if (Object.values(formDataJson).some(val => Array.isArray(val))) {
        ...
        // TODO: Fix how empty array (e.g. colors[]) is parsed. It should not even come here.
