...
import { createNavigationLeaveEventName, NAVIGATED_EVENT, NAVIGATED_LEAVE_EVENT, UPDATED_EVENT } from './triggers.js';
...
export class Suunta {
    ...
    getRoute(routeQueryObject) {
        ...
        for (const matcherEntry of this.routeMatchers.entries()) {
            ...
            return {
                ...matchedRoute,
                properties: { ...matchedRoute.properties, ...match.groups },
                queryParameters,
                hash,
            };
        }
    ...
    async navigate(route, { pushState = true, isReRender = false, originalEvent = null } = {}) {
        ...
        if (isViewRoute(route)) {
            this.currentNavigationRenderStack.push({ route, eventTarget: new EventTarget() });
    ...
    async emptyStackEntryRenderTarget(stackEntry) {
        ...
        if (stackEntry.renderTarget) {
            await this.options.renderer(viewToRender, routeToRender, stackEntry.renderTarget, false);
        }
    }
    ...
    manageEmptiedStackEntryState(stackEntry) {
        const state = stackEntry.viewState;
    ...
    async handleChildRoute(route) {
        ...
        for (let i = 0; i < routeStack.length; i++) {
            ...
            if (isViewRoute(routeToRender)) {
                this.currentNavigationRenderStack.push({ route: routeToRender, eventTarget: new EventTarget() });
    ...
    async handleViewRoute(route) {
        ...
        if (isChildRoute(route)) {
            ...
            const stackParent = this.getRenderStack().find(entry => entry.route.path === route.parent.path);
    ...
    render(viewToRender, route, parentRenderTarget) {
        ...
        this.options.renderer(viewToRender, route, target, false);
    }
     ...
     */
    triggerOnUpdated(stackEntry, propKey, oldValue, newValue) {
        stackEntry.eventTarget.dispatchEvent(
            new CustomEvent(UPDATED_EVENT, { detail: { property: propKey, oldValue, newValue } }),
        );
    }
}
