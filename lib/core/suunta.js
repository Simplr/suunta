...
import { createNavigationLeaveEventName, NAVIGATED_EVENT, onRender, RENDER_EVENT, UPDATED_EVENT } from './triggers.js';
...
export class Suunta {
    ...
    setupListeners() {
        document.body.addEventListener('click', clickEvent => {
            ...
            const closestLink = path
                ...
                .filter(el => el.href !== null && el.href !== undefined && el.href !== '')
                .pop();
            if (!closestLink || closestLink.download) {
                return;
            }
    ...
    async navigate(route, { pushState = true, isReRender = false, originalEvent = null } = {}) {
        ...
        let stackEntryWasRemoved = false;
        ...
        this.currentNavigationRenderStack.reverse().forEach(entry => {
            this.sendLeavingViewEvent(entry);
            this.manageEmptiedStackEntryState(entry);
        });
