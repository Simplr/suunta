...
export class Suunta {
    ...
    async emptyStackEntryRenderTarget(stackEntry) {
        ...
        if (stackEntry.renderTarget) {
            await this.options.renderer(viewToRender, routeToRender, stackEntry.renderTarget);
        }
    }
    ...
    async refreshView(stackEntry, previousEntry) {
        ...
        if (target && target.isConnected) {
            await this.options.renderer(renderResult, /** @type import('./route.js').ViewRoute */ (route), target);
        } else {
