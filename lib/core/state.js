...
function setupProxy(targetObject, router, propPath, stackEntry) {
    return new Proxy(targetObject, {
            ...
            try {
                ...
                if (typeof value === 'function' && Array.isArray(target)) {
                    if (['push', 'unshift', 'splice'].includes(prop)) {
                        // TODO: Write this to work with everything.
                        // We need to somehow make the types realize that this has those fields etc.
                        return (...args) => {
                            const wrapped = args.map(v =>
                                isObjectOrArray(v) && !v.isSuuntaProxy
                                    ? setupProxy(v, router, propPath, stackEntry)
                                    : v,
                            );
                            return value.apply(target, wrapped);
                        };
                    }
                }
        ...
        set(target, prop, value, receiver) {
            console.log('Proxied value updated');
            // @ts-ignore
